...
def train_epoch(epoch, wandb):
    ...
    if (not ddp or dist.get_rank() == 0):
        model.eval()
        moe_path = '_moe' if model_config.use_moe else ''
        ckp = f'{args.save_dir}/sft_vlm_ep{epoch+1}_{model_config.hidden_size}{moe_path}.pth'
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            state_dict = model.module.state_dict()
        else:
            state_dict = model.state_dict()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/root/minimind-v/dataset/sft_data.jsonl")
    parser.add_argument("--images_path", type=str, default="/root/minimind-v/dataset/sft_images")
